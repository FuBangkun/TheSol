...
public class ChunkProviderSedna extends ChunkProviderBase {
    public static final IBlockState BLOCK_TOP_DEFAULT = SolBlocks.SEDNA_TURF.getDefaultState();
    public static final IBlockState BLOCK_TOP_DARK = SolBlocks.SEDNA_SURFACE_ROCK.getDefaultState();
    public static final IBlockState BLOCK_FILL = SolBlocks.SEDNA_SUB_SURFACE_ROCK.getDefaultState();
    public static final IBlockState BLOCK_LOWER = SolBlocks.SEDNA_ROCK.getDefaultState();
    ...
    private final MapGenDungeonSedna dungeonGenerator = new MapGenDungeonSedna(new DungeonConfiguration(SolBlocks.SEDNA_DUNGEON_BRICK.getDefaultState(), 25, 8, 16, 5, 6, RoomBossUniversal.class, RoomTreasureSedna.class));
    ...
    public void setBlocksInChunk(int chunkX, int chunkZ, ChunkPrimer primer) {
        ...
        for (int x = 0; x < ChunkProviderSedna.CHUNK_SIZE_X; x++) {
            for (int z = 0; z < ChunkProviderSedna.CHUNK_SIZE_Z; z++) {
                final double d = this.noiseGen1.getNoise(x + chunkX * 16, z + chunkZ * 16) * 52;
                ...
                for (int y = 0; y < ChunkProviderSedna.CHUNK_SIZE_Y; y++) {
                    if (y < ChunkProviderSedna.MID_HEIGHT + yDev) {
    ...
    private void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
                for (int x = 0; x < ChunkProviderSedna.CHUNK_SIZE_X; x++) {
                    for (int z = 0; z < ChunkProviderSedna.CHUNK_SIZE_Z; z++) {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderSedna.CHUNK_SIZE_X + x, cz * ChunkProviderSedna.CHUNK_SIZE_Z + z) / ChunkProviderSedna.CRATER_PROB) {
