package mod.sol.planets.uranus.moon.ariel.world.gen;
...
public class ChunkProviderAriel extends ChunkProviderBase {
    public static final IBlockState BLOCK_TOP = SolBlocks.ARIEL_TURF.getDefaultState();
    public static final IBlockState BLOCK_FILL = SolBlocks.ARIEL_DIRT.getDefaultState();
    public static final IBlockState BLOCK_LOWER = SolBlocks.ARIEL_ROCK.getDefaultState();
    ...
    public ChunkProviderAriel(World par1World, long par2, boolean par4) {
        ...
        this.noiseGen1 = new Gradient(this.rand.nextLong(), 4, 0.15F);
        this.noiseGen2 = new Gradient(this.rand.nextLong(), 4, 0.15F);
        this.noiseGen3 = new Gradient(this.rand.nextLong(), 1, 0.15F);
        this.noiseGen4 = new Gradient(this.rand.nextLong(), 1, 0.15F);
    }
    ...
    public void setBlocksInChunk(int chunkX, int chunkZ, ChunkPrimer primer) {
        ...
        for (int x = 0; x < ChunkProviderAriel.CHUNK_SIZE_X; x++) {
            for (int z = 0; z < ChunkProviderAriel.CHUNK_SIZE_Z; z++) {
                ...
                for (int y = 0; y < ChunkProviderAriel.CHUNK_SIZE_Y; y++) {
                    if (y < ChunkProviderAriel.MID_HEIGHT + yDev) {
    ...
    private void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
                for (int x = 0; x < ChunkProviderAriel.CHUNK_SIZE_X; x++) {
                    for (int z = 0; z < ChunkProviderAriel.CHUNK_SIZE_Z; z++) {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderAriel.CHUNK_SIZE_X + x, cz * ChunkProviderAriel.CHUNK_SIZE_Z + z) / ChunkProviderAriel.CRATER_PROB) {
