...
public abstract class MapGenCavesSolBase extends MapGenBaseMeta {

    protected abstract Block[] getBreakableBlocks();

    protected IBlockState getReplacementBlock() {
        ...
        for (final boolean flag1 = random.nextInt(6) == 0; par15 < par16; ++par15) {
            ...
            if (flag || random.nextInt(4) != 0) {
                ...
                if (par6 >= d4 - 16.0D - d6 * 2.0D && par10 >= d5 - 16.0D - d6 * 2.0D && par6 <= d4 + 16.0D + d6 * 2.0D && par10 <= d5 + 16.0D + d6 * 2.0D) {
                    ...
                    Block[] breakableBlocks = getBreakableBlocks();
                    IBlockState replacement = getReplacementBlock();
                    int liquidLevel = getLiquidLevel();
                    IBlockState liquidBlock = getLiquidBlock();
                                        ...
                                        for (Block block : breakableBlocks) {
                                            if (state.getBlock() == block) {
                                                primer.setBlockState(localX, localY, localZ,
                                                        (liquidLevel >= 0 && localY <= liquidLevel) ? liquidBlock : replacement);
                                                break;
                                            }
                                        }
                                    }
