...
public class ChunkProviderEuropa extends ChunkProviderSolAdvanced {
    public ChunkProviderEuropa(World world, long seed) {
        super(world, seed);
    }

    @Override
    protected IBlockState getFillBlock() {
        return SolBlocks.EUROPA_ROCK.getDefaultState();
    }

    @Override
    protected MapGenBaseMeta getCaveGenerator() {
        return new MapGenCavesEuropa();
    }
            ...
            for (int z = 0; z < 16; ++z) {
                for (int y = 0; y < 127; ++y) {
                    if (primer.getBlockState(x, y, z) == SolBlocks.EUROPA_SUB_SURFACE_ROCK.getDefaultState() && primer.getBlockState(x, y + 1, z) == Blocks.AIR.getDefaultState()) {
                        ...
                        if (primer.getBlockState(x, y + 1, z) == Blocks.AIR.getDefaultState())
                            primer.setBlockState(x, y + 1, z, Blocks.SNOW_LAYER.getDefaultState());
                    }
                }
            }
        }
