...
public class ChunkProviderIo extends ChunkProviderSolNoise {
    public ChunkProviderIo(World world, long seed) {
        super(world, seed);
    }
    ...
    protected IBlockState getFillBlock(int x, int y, int z, Biome b) {
        return SolBlocks.IO_SUB_SURFACE_ROCK.getDefaultState();
    }

    @Override
    protected IBlockState getLowerBlock() {
        return SolBlocks.IO_ROCK.getDefaultState();
    }

    @Override
    protected MapGenBaseMeta createCaveGenerator() {
        return new MapGenCavesIo();
    }
