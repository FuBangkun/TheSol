package mod.sol.planets.saturn.moons.mimas.world.gen;
...
public class ChunkProviderMimas extends ChunkProviderBase {
    public static final IBlockState BLOCK_TOP = SolBlocks.MIMAS_TURF.getDefaultState();
    public static final IBlockState BLOCK_FILL = SolBlocks.MIMAS_DIRT.getDefaultState();
    public static final IBlockState BLOCK_LOWER = SolBlocks.MIMAS_ROCK.getDefaultState();
    ...
    public ChunkProviderMimas(World par1World, long par2, boolean par4) {
        ...
        this.noiseGen1 = new Gradient(this.rand.nextLong(), 4, 0.0425F);
        this.noiseGen2 = new Gradient(this.rand.nextLong(), 4, 0.0425F);
        this.noiseGen3 = new Gradient(this.rand.nextLong(), 1, 0.0425F);
        this.noiseGen4 = new Gradient(this.rand.nextLong(), 1, 0.0425F);
    }

    public void setBlocksInChunk(int chunkX, int chunkZ, ChunkPrimer primer) {
        this.noiseGen1.setFrequency(0.025F);
        this.noiseGen2.setFrequency(0.015F);
        this.noiseGen3.setFrequency(0.01F);
        this.noiseGen4.setFrequency(0.02F);

        for (int x = 0; x < ChunkProviderMimas.CHUNK_SIZE_X; x++) {
            for (int z = 0; z < ChunkProviderMimas.CHUNK_SIZE_Z; z++) {
                ...
                for (int y = 0; y < ChunkProviderMimas.CHUNK_SIZE_Y; y++) {
                    if (y < ChunkProviderMimas.MID_HEIGHT + yDev) {
    ...
    public Chunk generateChunk(int x, int z) {
        this.rand.setSeed((long) x * 541287152391L + (long) z * 132897987541L);
    ...
    private void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
                for (int x = 0; x < ChunkProviderMimas.CHUNK_SIZE_X; x++) {
                    for (int z = 0; z < ChunkProviderMimas.CHUNK_SIZE_Z; z++) {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderMimas.CHUNK_SIZE_X + x, cz * ChunkProviderMimas.CHUNK_SIZE_Z + z) / ChunkProviderMimas.CRATER_PROB) {
