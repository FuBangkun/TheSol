package mod.sol.planets.jupiter.moons.europa.world.gen;
...
import mod.sol.planets.jupiter.moons.europa.biome.BiomeDecoratorEuropa;
import mod.sol.planets.jupiter.moons.europa.biome.BiomeGenEuropaValley;
...
public class ChunkProviderEuropa extends ChunkProviderBase {
    ...
    public ChunkProviderEuropa(World worldIn, long seed, boolean mapFeaturesEnabled) {
        this.world = worldIn;
            ...
            for (int localZ = 0; localZ < 16; ++localZ) {
                for (int localY = 0; localY < 127; ++localY) {
                    if (localY <= level && primer.getBlockState(localX, localY, localZ) == Blocks.AIR.getDefaultState()) {
                        primer.setBlockState(localX, localY, localZ, Blocks.WATER.getDefaultState());
                    }
                    if (localY == level && primer.getBlockState(localX, localY, localZ) == Blocks.WATER.getDefaultState()) {
                        primer.setBlockState(localX, localY, localZ, Blocks.PACKED_ICE.getDefaultState());
                    }
                }
            }
        }
    ...
    private void generateSurface(ChunkPrimer primer) {
        for (int localX = 0; localX < 16; ++localX) {
            for (int localZ = 0; localZ < 16; ++localZ) {
                for (int localY = 0; localY < 127; ++localY) {
                    if (primer.getBlockState(localX, localY, localZ) == SolBlocks.EUROPA_SUB_SURFACE_ROCK.getDefaultState() && primer.getBlockState(localX, localY + 1, localZ) == Blocks.AIR.getDefaultState()) {
                        primer.setBlockState(localX, localY, localZ, SolBlocks.EUROPA_SURFACE_ROCK.getDefaultState());
                        for (int i = 0; i < rand.nextInt(4); ++i) {
                            primer.setBlockState(localX, localY - 1 - i, localZ, SolBlocks.EUROPA_SUB_SURFACE_ROCK.getDefaultState());
                        }
                    }
                }
            }
    ...
    public Chunk generateChunk(int x, int z) {
        this.rand.setSeed((long) x * 705199291138L + (long) z * 185336455139L);
...
//                    double d5 = MathHelper.clampedLerp(d2, d3, d4) - d1;
    ...
    public void populate(int x, int z) {
        ...
        boolean isValley = biomegenbase instanceof BiomeAdaptive && ((BiomeAdaptive) biomegenbase).isInstance(BiomeGenEuropaValley.class);
